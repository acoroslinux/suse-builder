...
class SystemCustomizer:
    ...
    def configure_system_defaults(self):
        ...
        # Ensure robust DNS configuration and DHCP auto-networking
        ...
        resolv_conf.write_text(
            "# Generated by suse-builder\n"
            "nameserver 1.1.1.1\n"
            "nameserver 8.8.8.8\n"
            "nameserver 9.9.9.9\n"
        )

        # Configure systemd-networkd for automatic DHCP on all interfaces
        net_dir = etc_dir / "systemd" / "network"
        net_dir.mkdir(parents=True, exist_ok=True)
        (net_dir / "99-dhcp.network").write_text(
            "[Match]\n"
            "Name=en* eth* wl*\n\n"
            "[Network]\n"
            "DHCP=yes\n"
        ...
        # Configure NetworkManager default settings
        nm_dir = etc_dir / "NetworkManager"
        nm_dir.mkdir(parents=True, exist_ok=True)
        (nm_dir / "NetworkManager.conf").write_text(
            "[main]\n"
            "plugins=keyfile\n"
            "dhcp=internal\n\n"
            "[ifupdown]\n"
    ...
    def setup_services(self):
        ...
        # Auto-enable NetworkManager and systemd-networkd for out-of-the-box networking
        for net_svc in ["NetworkManager", "systemd-networkd", "dbus", "sshd", "qemu-guest-agent"]:
            if net_svc not in services_to_enable:
                for search_dir in ["usr/lib/systemd/system", "lib/systemd/system"]:
                    if (self.target_root / search_dir / f"{net_svc}.service").exists():
                        services_to_enable.append(net_svc)
                        break
